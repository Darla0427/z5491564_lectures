...
def get_tics(pth):
    ...
    with open(pth, mode='r') as fobj:
        ...
        for line in fobj:
            ...
            if len(parts) == 2:
                exchange, tic = parts
                if exchange.isalpha() and tic.isalpha() and exchange and tic:
                    data_dict[tic] = exchange
    return data_dict
...
def line_to_dict(line):
    ...
    for column in COLUMNS:
        data_dict[column] = line[i:i + COLWIDTHS[column]].strip()
        i += COLWIDTHS[column]
    return data_dict
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    if tickers_lst is None:
        ...
        invalid_tickers = [ticker for ticker in tickers_lst if ticker.lower() not in tic_exchange_dic]
        if invalid_tickers:
            raise Exception(f'Tickers not found in tic_exchange_dic: {", ".join(invalid_tickers)}')
...
def verify_cols(col_lst=None):
    ...
    if col_lst is None:
        ...
        invalid_cols = [col for col in col_lst if col not in COLUMNS]
        if invalid_cols:
            raise Exception(f'Columns not found in COLUMNS: {", ".join(invalid_cols)}')
